# NOTE: This class is auto generated by the swagger code generator program.
# https://github.com/swagger-api/swagger-codegen.git
# Do not edit the class manually.

defmodule EvaluationsAPI.Api.Organisations do
  @moduledoc """
  API calls for all endpoints tagged `Organisations`.
  """

  alias EvaluationsAPI.Connection
  import EvaluationsAPI.RequestBuilder


  @doc """
  Delete an Organisation

  ## Parameters

  - connection (EvaluationsAPI.Connection): Connection to server
  - organisation_id (integer()): Organisation identifier
  - opts (KeywordList): [optional] Optional parameters

  ## Returns

  {:ok, %{}} on success
  {:error, info} on failure
  """
  @spec delete_organisation_dao(Tesla.Env.client, integer(), keyword()) :: {:ok, nil} | {:error, Tesla.Env.t}
  def delete_organisation_dao(connection, organisation_id, _opts \\ []) do
    %{}
    |> method(:delete)
    |> url("/organisations/#{organisation_id}")
    |> Enum.into([])
    |> (&Connection.request(connection, &1)).()
    |> decode(false)
  end

  @doc """
  Get information of an organisation

  ## Parameters

  - connection (EvaluationsAPI.Connection): Connection to server
  - organisation_id (integer()): Organisation identifier
  - opts (KeywordList): [optional] Optional parameters
    - :x_fields (String.t): An optional fields mask

  ## Returns

  {:ok, %EvaluationsAPI.Model.Organisation{}} on success
  {:error, info} on failure
  """
  @spec get_organisation_dao(Tesla.Env.client, integer(), keyword()) :: {:ok, EvaluationsAPI.Model.Organisation.t} | {:error, Tesla.Env.t}
  def get_organisation_dao(connection, organisation_id, opts \\ []) do
    optional_params = %{
      :"X-Fields" => :headers
    }
    %{}
    |> method(:get)
    |> url("/organisations/#{organisation_id}")
    |> add_optional_params(optional_params, opts)
    |> Enum.into([])
    |> (&Connection.request(connection, &1)).()
    |> decode(%EvaluationsAPI.Model.Organisation{})
  end

  @doc """
  Get all organisations

  ## Parameters

  - connection (EvaluationsAPI.Connection): Connection to server
  - opts (KeywordList): [optional] Optional parameters
    - :x_fields (String.t): An optional fields mask

  ## Returns

  {:ok, [%Organisation{}, ...]} on success
  {:error, info} on failure
  """
  @spec get_organisation_list_dao(Tesla.Env.client, keyword()) :: {:ok, list(EvaluationsAPI.Model.Organisation.t)} | {:error, Tesla.Env.t}
  def get_organisation_list_dao(connection, opts \\ []) do
    optional_params = %{
      :"X-Fields" => :headers
    }
    %{}
    |> method(:get)
    |> url("/organisations/")
    |> add_optional_params(optional_params, opts)
    |> Enum.into([])
    |> (&Connection.request(connection, &1)).()
    |> decode([%EvaluationsAPI.Model.Organisation{}])
  end

  @doc """
  Create a new organisation

  ## Parameters

  - connection (EvaluationsAPI.Connection): Connection to server
  - payload (Organisation): 
  - opts (KeywordList): [optional] Optional parameters
    - :x_fields (String.t): An optional fields mask

  ## Returns

  {:ok, %EvaluationsAPI.Model.Organisation{}} on success
  {:error, info} on failure
  """
  @spec post_organisation_list_dao(Tesla.Env.client, EvaluationsAPI.Model.Organisation.t, keyword()) :: {:ok, EvaluationsAPI.Model.Organisation.t} | {:error, Tesla.Env.t}
  def post_organisation_list_dao(connection, payload, opts \\ []) do
    optional_params = %{
      :"X-Fields" => :headers
    }
    %{}
    |> method(:post)
    |> url("/organisations/")
    |> add_param(:body, :"payload", payload)
    |> add_optional_params(optional_params, opts)
    |> Enum.into([])
    |> (&Connection.request(connection, &1)).()
    |> decode(%EvaluationsAPI.Model.Organisation{})
  end

  @doc """
  Update an Organisation

  ## Parameters

  - connection (EvaluationsAPI.Connection): Connection to server
  - organisation_id (integer()): Organisation identifier
  - payload (Organisation): 
  - opts (KeywordList): [optional] Optional parameters
    - :x_fields (String.t): An optional fields mask

  ## Returns

  {:ok, %EvaluationsAPI.Model.Organisation{}} on success
  {:error, info} on failure
  """
  @spec put_organisation_dao(Tesla.Env.client, integer(), EvaluationsAPI.Model.Organisation.t, keyword()) :: {:ok, EvaluationsAPI.Model.Organisation.t} | {:error, Tesla.Env.t}
  def put_organisation_dao(connection, organisation_id, payload, opts \\ []) do
    optional_params = %{
      :"X-Fields" => :headers
    }
    %{}
    |> method(:put)
    |> url("/organisations/#{organisation_id}")
    |> add_param(:body, :"payload", payload)
    |> add_optional_params(optional_params, opts)
    |> Enum.into([])
    |> (&Connection.request(connection, &1)).()
    |> decode(%EvaluationsAPI.Model.Organisation{})
  end

  @doc """
  Add or subtract quota for an organisation

  ## Parameters

  - connection (EvaluationsAPI.Connection): Connection to server
  - organisation_id (integer()): Organisation identifier
  - payload (OrganisationQuota): 
  - opts (KeywordList): [optional] Optional parameters

  ## Returns

  {:ok, %{}} on success
  {:error, info} on failure
  """
  @spec put_quota_dao(Tesla.Env.client, integer(), EvaluationsAPI.Model.OrganisationQuota.t, keyword()) :: {:ok, nil} | {:error, Tesla.Env.t}
  def put_quota_dao(connection, organisation_id, payload, _opts \\ []) do
    %{}
    |> method(:put)
    |> url("/organisations/addquota/#{organisation_id}")
    |> add_param(:body, :"payload", payload)
    |> Enum.into([])
    |> (&Connection.request(connection, &1)).()
    |> decode(false)
  end
end
