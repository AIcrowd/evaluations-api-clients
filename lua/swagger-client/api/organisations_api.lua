...
local swagger-client_organisation_quota = require "swagger-client.model.organisation_quota"

local organisations_api = {}
local organisations_api_mt = {
	__name = "organisations_api";
	__index = organisations_api;
}

local function new_organisations_api(host, basePath, schemes)
	local schemes_map = {}
...
function organisations_api:delete_organisation_dao(organisation_id)
	local req = http_request.new_from_uri({
		...
		host = self.host;
		path = string.format("%s/organisations/%s",
...
function organisations_api:get_organisation_dao(organisation_id, x_fields)
	...
	if http_status:sub(1,1) == "2" then
		...
		return swagger-client_organisation.cast(result), headers
	else
		local body, err, errno2 = stream:get_body_as_string()
		if not body then
			return nil, err, errno2
...
function organisations_api:get_organisation_list_dao(x_fields)
	...
	if http_status:sub(1,1) == "2" then
		...
		for _, ob in ipairs(result) do
			swagger-client_organisation.cast(ob)
		end
