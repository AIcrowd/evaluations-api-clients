...
function submissions_api:delete_submission_dao(submission_id)
...
function submissions_api:get_submission_dao(submission_id, x_fields)
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
		path = string.format("%s/submissions/%s",
	...
	if http_status:sub(1,1) == "2" then
		...
		return swagger-client_submissions.cast(result), headers
	else
		local body, err, errno2 = stream:get_body_as_string()
		if not body then
			return nil, err, errno2
...
function submissions_api:get_submission_list_dao(x_fields)
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
	...
	if http_status:sub(1,1) == "2" then
		...
		for _, ob in ipairs(result) do
			swagger-client_submissions.cast(ob)
		end
...
function submissions_api:post_submission_list_dao(payload, x_fields)
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
		path = string.format("%s/submissions",
