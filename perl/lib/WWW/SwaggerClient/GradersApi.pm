...
#
package WWW::SwaggerClient::GradersApi;
...
# @param string $x_fields An optional fields mask (optional)
{
    ...
    __PACKAGE__->method_documentation->{ 'put_grader_dao' } = { 
    	summary => '',
        params => $params,
        returns => 'Grader',
        };
...
sub put_grader_dao {
    ...
    # verify the required parameter 'grader_id' is set
    unless (exists $args{'grader_id'}) {
      croak("Missing the required parameter 'grader_id' when calling put_grader_dao");
    }

    # verify the required parameter 'payload' is set
    unless (exists $args{'payload'}) {
      croak("Missing the required parameter 'payload' when calling put_grader_dao");
    }
