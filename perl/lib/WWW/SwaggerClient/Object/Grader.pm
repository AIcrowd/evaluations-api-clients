=begin comment

Evaluations API

API to create and evaluate custom challenges

OpenAPI spec version: 1.0.0

Generated by: https://github.com/swagger-api/swagger-codegen.git

=end comment

=cut

#
# NOTE: This class is auto generated by the swagger code generator program. 
# Do not edit the class manually.
# Ref: https://github.com/swagger-api/swagger-codegen
#
package WWW::SwaggerClient::Object::Grader;

require 5.6.0;
use strict;
use warnings;
use utf8;
use JSON qw(decode_json);
use Data::Dumper;
use Module::Runtime qw(use_module);
use Log::Any qw($log);
use Date::Parse;
use DateTime;


use base ("Class::Accessor", "Class::Data::Inheritable");


#
#
#
# NOTE: This class is auto generated by the swagger code generator program. Do not edit the class manually.
# REF: https://github.com/swagger-api/swagger-codegen
#

=begin comment

Evaluations API

API to create and evaluate custom challenges

OpenAPI spec version: 1.0.0

Generated by: https://github.com/swagger-api/swagger-codegen.git

=end comment

=cut

#
# NOTE: This class is auto generated by the swagger code generator program. 
# Do not edit the class manually.
# Ref: https://github.com/swagger-api/swagger-codegen
#
__PACKAGE__->mk_classdata('attribute_map' => {});
__PACKAGE__->mk_classdata('swagger_types' => {});
__PACKAGE__->mk_classdata('method_documentation' => {}); 
__PACKAGE__->mk_classdata('class_documentation' => {});

# new object
sub new { 
    my ($class, %args) = @_; 

	my $self = bless {}, $class;
	
	foreach my $attribute (keys %{$class->attribute_map}) {
		my $args_key = $class->attribute_map->{$attribute};
		$self->$attribute( $args{ $args_key } );
	}
	
	return $self;
}  

# return perl hash
sub to_hash {
    return decode_json(JSON->new->convert_blessed->encode( shift ));
}

# used by JSON for serialization
sub TO_JSON { 
    my $self = shift;
    my $_data = {};
    foreach my $_key (keys %{$self->attribute_map}) {
        if (defined $self->{$_key}) {
            $_data->{$self->attribute_map->{$_key}} = $self->{$_key};
        }
    }
    return $_data;
}

# from Perl hashref
sub from_hash {
    my ($self, $hash) = @_;

    # loop through attributes and use swagger_types to deserialize the data
    while ( my ($_key, $_type) = each %{$self->swagger_types} ) {
    	my $_json_attribute = $self->attribute_map->{$_key}; 
        if ($_type =~ /^array\[/i) { # array
            my $_subclass = substr($_type, 6, -1);
            my @_array = ();
            foreach my $_element (@{$hash->{$_json_attribute}}) {
                push @_array, $self->_deserialize($_subclass, $_element);
            }
            $self->{$_key} = \@_array;
        } elsif (exists $hash->{$_json_attribute}) { #hash(model), primitive, datetime
            $self->{$_key} = $self->_deserialize($_type, $hash->{$_json_attribute});
        } else {
        	$log->debugf("Warning: %s (%s) does not exist in input hash\n", $_key, $_json_attribute);
        }
    }
  
    return $self;
}

# deserialize non-array data
sub _deserialize {
    my ($self, $type, $data) = @_;
    $log->debugf("deserializing %s with %s",Dumper($data), $type);
        
    if ($type eq 'DateTime') {
        return DateTime->from_epoch(epoch => str2time($data));
    } elsif ( grep( /^$type$/, ('int', 'double', 'string', 'boolean'))) {
        return $data;
    } else { # hash(model)
        my $_instance = eval "WWW::SwaggerClient::Object::$type->new()";
        return $_instance->from_hash($data);
    }
}



__PACKAGE__->class_documentation({description => '',
                                  class => 'Grader',
                                  required => [], # TODO
}                                 );

__PACKAGE__->method_documentation({
    'id' => {
    	datatype => 'int',
    	base_name => 'id',
    	description => 'ID',
    	format => '',
    	read_only => '',
    		},
    'created' => {
    	datatype => 'DateTime',
    	base_name => 'created',
    	description => 'Creation time',
    	format => '',
    	read_only => '',
    		},
    'updated' => {
    	datatype => 'DateTime',
    	base_name => 'updated',
    	description => 'Last updation time',
    	format => '',
    	read_only => '',
    		},
    'dataset_url' => {
    	datatype => 'string',
    	base_name => 'dataset_url',
    	description => 'S3 link of the Dataset',
    	format => '',
    	read_only => '',
    		},
    'code_access_mode' => {
    	datatype => 'string',
    	base_name => 'code_access_mode',
    	description => 'git/http',
    	format => '',
    	read_only => '',
    		},
    'code_access_auth_key' => {
    	datatype => 'string',
    	base_name => 'code_access_auth_key',
    	description => 'SSH private key if using git or HTTP Auth token if using HTTP to access the submission code',
    	format => '',
    	read_only => '',
    		},
    'cluster_id' => {
    	datatype => 'int',
    	base_name => 'cluster_id',
    	description => 'Cluster to run the grader on',
    	format => '',
    	read_only => '',
    		},
    'docker_username' => {
    	datatype => 'string',
    	base_name => 'docker_username',
    	description => 'Docker registry username',
    	format => '',
    	read_only => '',
    		},
    'docker_password' => {
    	datatype => 'string',
    	base_name => 'docker_password',
    	description => 'Docker registry password',
    	format => '',
    	read_only => '',
    		},
    'docker_registry' => {
    	datatype => 'string',
    	base_name => 'docker_registry',
    	description => 'Docker registry URL. Dockerhub is used by default.',
    	format => '',
    	read_only => '',
    		},
    'workflow_spec' => {
    	datatype => 'object',
    	base_name => 'workflow_spec',
    	description => 'Argo workflow template spec',
    	format => '',
    	read_only => '',
    		},
    'evaluation_code' => {
    	datatype => 'string',
    	base_name => 'evaluation_code',
    	description => 'S3 link to the zip file containing the code that will be used for the evaluation',
    	format => '',
    	read_only => '',
    		},
    'storage_capacity' => {
    	datatype => 'string',
    	base_name => 'storage_capacity',
    	description => 'Size of the dataset partition to request. Please provide at least 2x of the size of the dataset.',
    	format => '',
    	read_only => '',
    		},
    'meta' => {
    	datatype => 'object',
    	base_name => 'meta',
    	description => 'Additional meta data of the grader',
    	format => '',
    	read_only => '',
    		},
    'status' => {
    	datatype => 'boolean',
    	base_name => 'status',
    	description => 'Status of the grader - True if it ready, False otherwise',
    	format => '',
    	read_only => '',
    		},
    'user_id' => {
    	datatype => 'int',
    	base_name => 'user_id',
    	description => 'User ID',
    	format => '',
    	read_only => '',
    		},
    'organisation_id' => {
    	datatype => 'int',
    	base_name => 'organisation_id',
    	description => 'Organisation ID',
    	format => '',
    	read_only => '',
    		},
});

__PACKAGE__->swagger_types( {
    'id' => 'int',
    'created' => 'DateTime',
    'updated' => 'DateTime',
    'dataset_url' => 'string',
    'code_access_mode' => 'string',
    'code_access_auth_key' => 'string',
    'cluster_id' => 'int',
    'docker_username' => 'string',
    'docker_password' => 'string',
    'docker_registry' => 'string',
    'workflow_spec' => 'object',
    'evaluation_code' => 'string',
    'storage_capacity' => 'string',
    'meta' => 'object',
    'status' => 'boolean',
    'user_id' => 'int',
    'organisation_id' => 'int'
} );

__PACKAGE__->attribute_map( {
    'id' => 'id',
    'created' => 'created',
    'updated' => 'updated',
    'dataset_url' => 'dataset_url',
    'code_access_mode' => 'code_access_mode',
    'code_access_auth_key' => 'code_access_auth_key',
    'cluster_id' => 'cluster_id',
    'docker_username' => 'docker_username',
    'docker_password' => 'docker_password',
    'docker_registry' => 'docker_registry',
    'workflow_spec' => 'workflow_spec',
    'evaluation_code' => 'evaluation_code',
    'storage_capacity' => 'storage_capacity',
    'meta' => 'meta',
    'status' => 'status',
    'user_id' => 'user_id',
    'organisation_id' => 'organisation_id'
} );

__PACKAGE__->mk_accessors(keys %{__PACKAGE__->attribute_map});


1;
